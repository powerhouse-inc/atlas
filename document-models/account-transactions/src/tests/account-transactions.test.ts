...
describe("AccountTransactions Operations", () => {
  ...
  it("should handle updateTransaction operation", () => {
    ...
    expect(updatedDocument.state.global.transactions[0].amount).toEqual(
      updateInput.amount,
    );
    expect(updatedDocument.state.global.transactions[0].details).toEqual(
      updateInput.details,
    );
    // Verify unchanged fields remain the same
    expect(updatedDocument.state.global.transactions[0].fromAccount).toBe(
      createInput.fromAccount,
    );
    expect(updatedDocument.state.global.transactions[0].toAccount).toBe(
      createInput.toAccount,
    );
  });
  ...
  it("should handle updateTransactionBudget operation", () => {
    ...
    expect(updatedDocument.operations.global[1].index).toEqual(1);
    expect(updatedDocument.state.global.transactions[0].budget).toBe(
