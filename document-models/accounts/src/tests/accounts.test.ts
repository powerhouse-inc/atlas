...
describe("Accounts Operations", () => {
  ...
  it("should handle updateAccount operation", () => {
    ...
    expect(updatedDocument.state.global.accounts[0].budgetPath).toBe(
      updateInput.budgetPath,
    );
    // Verify unchanged fields remain the same
    expect(updatedDocument.state.global.accounts[0].account).toBe(
      createInput.account,
    );
    expect(updatedDocument.state.global.accounts[0].type).toBe(
      createInput.type,
    );
  });
