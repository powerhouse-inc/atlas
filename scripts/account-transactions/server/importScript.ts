...
export const importTransactions = async (
  ...
  try {
    ...
    if (!rootDirId) {
      throw new Error(
        "Failed to create or find Account Transactions directory",
      );
    }
    ...
    const newTransactions = incomingTransactions.erc20Transfers.filter(
      (transaction) => {
        return !docTransactions.some(
          (docTx: any) => docTx.details.crypto.txHash === transaction.txHash,
        );
      },
    );
    ...
    if (newTransactions.length > 0) {
      console.log(
        `Adding ${newTransactions.length} new transactions to document`,
      );
